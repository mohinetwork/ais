...
export default function AnmixDashboard() {
  ...
  const [selectedModel, setSelectedModel] = useState("FYOIA-V1");
  ...
  const VIDEO_MODELS_FOR_CHAT = ["FYOIA-VID"];
  ...
  const handleSendMessage = async (customMsg?: string, previews?: FilePreview[]) => {
    ...
    if (chatMode === "image-gen" || chatMode === "image-edit") {
      ...
      const modelMap: Record<string, string> = {
        "FYOIA-IMG": "gptimage",
        "FYOIA-IMG-GEN": "imagen-4",
        "FYOIA-BETA": "grok-imagine",
        "FYOIA-IMG-V2": "klein",
        "FYOIA-IMG-PRO": "klein-large",
      };
    ...
    if (chatMode === "video-gen") {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(typeof errData?.error === "string" ? errData.error : "Video generation failed.");
        }
          ...
          while (true) {
            ...
            for (const part of parts) {
              ...
              try {
                ...
                if (evt.status === "done" && evt.url) { videoUrl = evt.url; break; }
                if (evt.status === "error") throw new Error(evt.error || "Video generation failed.");
              } catch (parseErr) {
    ...
    // Use Groq API for all AI chat models (FYOIA-V1, FYOIA-THINK)
  ...
  const startImageChat = () => {
    ...
    setSelectedModel("FYOIA-IMG-PRO");
  ...
  const buildGeneratedFilename = (prompt: string) => {
    ...
    return `FYOIA-AI-${kw}.png`;
  };

  const buildGeneratedVideoFilename = (prompt: string) => {
    const kw = extractPromptKeywords(prompt, 3);
    return `FYOIA-AI-VIDEO-${kw}.mp4`;
  };
  ...
  const downloadGeneratedImage = async (url: string, prompt: string) => {
    try {
      ...
      if (!baseRes.ok) throw new Error("failed to fetch image");
      const baseBlob = await baseRes.blob();

      const baseObjUrl = URL.createObjectURL(baseBlob);
      const baseImg = await loadImage(baseObjUrl);
      ...
      for (const wmPath of ["/anmixai/fyoia-ai-watermark.png", "/fyoia-ai-watermark.png"]) {
        try {
          const wmRes = await fetch(wmPath);
          if (!wmRes.ok) continue;
          const wmBlob = await wmRes.blob();
          const wmObjUrl = URL.createObjectURL(wmBlob);
          const wmImg = await loadImage(wmObjUrl);
          const scale = 0.14;
          ...
          ctx.drawImage(wmImg, canvas.width - targetW - pad, canvas.height - targetH - pad, targetW, targetH);
          ctx.restore();
      ...
      if (!finalBlob) throw new Error("toBlob failed");
    ...
    <div className="min-h-screen text-foreground bg-[#010104] dark:bg-[#010104] font-sans selection:bg-[#0055FF]/30 overflow-hidden relative">
          ...
          <>
            <div className="p-4">
              <EyeCatchingButton_v1
                onClick={clearChat}
                className="w-full group relative overflow-hidden rounded-[18px] bg-[#101624]/95 border border-white/10 backdrop-blur-xl px-4 py-3.5 flex items-center justify-center gap-2.5 text-sm font-semibold text-white/95 shadow-[0_18px_45px_rgba(15,23,42,0.9)] hover:bg-[#111827]/95 hover:border-white/20 transition-all"
              >
                <Plus size={20} />
                <span>New Chat</span>
              </EyeCatchingButton_v1>
                  ...
                  <div className="mt-2">
                    ...
                    >
                      <ShimmerButton
                        ...
                        className="px-4 py-1.5 text-[11px] w-full"
                      >
                        Upgrade Pro →
                      </ShimmerButton>
                    </div>
      ...
      <main className="flex-1 flex flex-col relative z-10 overflow-hidden min-w-0">
        <header className="relative h-14 flex items-center justify-between px-4 sm:px-6 shrink-0">
          ...
          <div ref={scrollRef} className={`flex-1 ${messages.length === 0 ? "overflow-y-hidden" : "overflow-y-auto scrollbar-hide scroll-smooth"}`}>
            <div className={`max-w-4xl mx-auto px-4 space-y-1.5 ${messages.length === 0 ? "h-full flex flex-col justify-center py-3" : "py-2"}`}>
              ...
              {messages.length === 0 ? (
                ...
                >
                  <div className="relative group overflow-visible">
                    <div className="absolute -inset-3 bg-[#0055FF]/10 rounded-full blur-3xl group-hover:bg-[#0055FF]/15 transition-all duration-700" />
                    <div className="relative z-10 overflow-visible">
                      <AILoader size={160} text="Fyoia AI" responsive />
                    </div>
                  </div>
                  <div className="space-y-4">
                    ...
                    <p className="text-[9px] sm:text-[10px] md:text-xs text-white/50 font-medium tracking-[0.15em] uppercase whitespace-nowrap overflow-x-auto max-w-[95vw] scrollbar-hide">
                      All in one open source ai - Fyoia{"\u00A0"}AI
                    </p>
                  </div>
                  {voiceMode && (
                    <div className="flex flex-wrap items-center gap-3 justify-center text-xs">
                      <GradientButton
                        ...
                        className="text-[11px] sm:text-xs px-4 py-2 min-w-[150px] justify-center"
                      />
                      <GradientButton
                        onClick={() => {
                          setVoiceAgentActive(false);
                        ...
                        variant="purple"
                        label="Close"
                        className="text-[11px] sm:text-xs px-4 py-2 min-w-[110px] justify-center"
                      />
                    </div>
                  )}
                </motion.div>
                ...
                >
                  ...
                  {isTyping && selectedModel !== "FYOIA-THINK" && (
                    <motion.div 
        ...
        <div className="pb-6 sm:pb-8 lg:pb-10 px-4 sm:px-6 lg:px-10">
          <div className="max-w-4xl mx-auto relative group">
            ...
            <p className="text-[10px] text-center text-white/40 mt-6 uppercase tracking-[0.2em] font-medium">
                Fyoia AI can make mistakes. Please double check the answers.
            </p>
          </div>
        </div>
        )}
      ...
      {/* Auth dialog removed - free access */}
      {/* Login removed - Fyoia AI is free */}
          ...
          <div className="flex flex-col gap-2">
            ...
            <DialogHeader>
              ...
              <DialogDescription className="text-left">
                Manage the card used for your Fyoia AI subscription. This is demo UI only.
              </DialogDescription>
            </DialogHeader>
          </div>
